...
class UserParams(BaseModel):
    ...
    def validate_name(cls, value):
        if len(value) > 50:
            raise HTTPException(detail={"name": "Seu nome deve conter até 50 caracteres"}, status_code=500)
        if not value.strip():
            raise HTTPException(detail={"name": "Por favor, digite um nome"}, status_code=500)
        return value
