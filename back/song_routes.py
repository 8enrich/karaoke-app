...
async def create_song(params: SongParams, key: str = Depends(verify_key)):
    ...
    song = await SongDb.create(params.name, params.link, key, params.artist, params.sing)
...
@router.put("/{id}")
async def update_song(params: UpdateSongParams, id: int, key: str = Depends(verify_key)):
    user = await get_user(key)
